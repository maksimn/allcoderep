...
public class FindToXmlTransformation : ITransformation {
    ...
    private Boolean IsFile(String path, Int32 pos) {
        for (Int32 i = pos + 1; i < find.n; i++) {
            if (find.filepath[i].StartsWith(path)) {
                return false;
            }
        }
        return true;
